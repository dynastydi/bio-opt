...
class PSO:
    ...
    def run(self, func, iters):
        ...
        for i in range(iters):
            ...
            self.GEN += 1
            np.save(f'milestones/pso/i', self.informers)
            np.save(f'milestones/pso/x_{self.GEN}', self.particles)
            np.save(f'milestones/pso/v_{self.GEN}', self.velocity)
            np.save(f'milestones/pso/pb_{self.GEN}', self.personal_best)
            np.save(f'milestones/pso/pl_{self.GEN}', self.personal_location)

        return score

    def load(self, gen:int):
        self.GEN = gen
        self.informers = np.load(f'milestones/pso/i.npy')
        self.particles = np.load(f'milestones/pso/x_{self.GEN}.npy')
        self.velocity = np.load(f'milestones/pso/v_{self.GEN}.npy')
        self.personal_best = np.load(f'milestones/pso/pb_{self.GEN}.npy')
        self.personal_location = np.load(f'milestones/pso/pl_{self.GEN}.npy')
...
class GA:
    ...
    # run an optimisation with 'iters' iterations
    def run(self, iters):
        ...
        for i in range(iters):
            score = self._step()
            self.GEN += 1
            np.save(f'milestones/ga/x_{self.GEN}', self.individuals)
            np.save(f'milestones/ga/y_{self.GEN}', self.fitness)
        return score
    
    # load from 'gen' generation saved from previous run
    def load(self, gen:int):
        self.GEN = gen
        self.individuals = np.load(f'milestones/ga/x_{self.GEN}.npy')
        self.fitness = np.load(f'milestones/ga/y_{self.GEN}.npy')
    
    # create population before a run
    def spawn(self):
...
### ARTIFICAL IMMUNE SYSTEM - CLONAL SELECTION ALGORITHM (CLONALG)

class AIS:

    def __init__(self,
                 func,                  # problem to be optimised
                 size: int,             # size of the population
                 variables: int,        # number of problem dimensions
                 span: tuple,           # (min, max) of variable range
                 clones: int,           # number of clones per generation
                 mutation_decay: float, # amount of decay in mutation proportion throughout training
                 ):
    ...
    def run(self, iters):
        ...
        for i in range(iters):
            score = self._step()
            self.GEN += 1
            np.save(f'milestones/ais/x_{self.GEN}', self.antibodies)
            np.save(f'milestones/ais/y_{self.GEN}', self.fitness)
        return score
        
    # load from 'gen' generation saved from previous run
    def load(self, gen:int):
        self.GEN = gen
        self.antibodies = np.load(f'milestones/ais/x_{self.GEN}.npy')
        self.fitness = np.load(f'milestones/ais/y_{self.GEN}.npy')
    
    # create new population before a run
    def spawn(self):
    ...
    def _step(self):
        ...
        # select indices for the N best individuals
        source = np.squeeze(np.argsort(self.fitness, axis=0))
        
        # create array of mutations
        mutations = rng.normal(loc=0, scale=self.MUTATION_SCALE, size=(self.SIZE, self.VARS, self.CLONES))
        # create a mask for this array - mutations will occur at a rate inversely proportional to an individuals performance
        mutation_mask = rng.random((self.SIZE, self.VARS, self.CLONES)) < self.MUTATION_RATES
        
        # create population of clones, mutating them as above
        clones = self.antibodies[source] + (mutations * mutation_mask)
    
        # clip to be within range
        clones = np.clip(clones, self.FLOOR, self.CEIL)

        # run function and get scores
